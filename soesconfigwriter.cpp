	...
	if(!configout.fail()) {
		...
		for(SyncManager* sm : dev->syncmanagers) {
			...
			if(0 == strcmp(sm->type,"Outputs")) { // TODO verify that the actual assigned SyncManager *is* 2
				...
				if(NULL != dev->slots) {
					dynrxpdo += dev->slots->maxslotcount;
				}
			...
			if(0 == strcmp(sm->type,"Inputs")) { // TODO verify that the actual assigned SyncManager *is* 3
				...
				if(NULL != dev->slots) {
					dyntxpdo += dev->slots->maxslotcount;
				}
	...
	if(dev->profile && dev->profile->dictionary) {
		...
		if(dev->modules) {
			...
			out << "/** Autogenerated by " << APP_NAME << " v" << APP_VERSION << " */\n"
			    << "#ifndef " << CNameify(dev->name,true) << "_MODULES_H\n"
			    << "#define " << CNameify(dev->name,true) << "_MODULES_H\n"
			    << "#include <stddef.h>\n"
			    << "#include <stdint.h>\n"
			    << "\n";
			...
			out << "#endif /* " << CNameify(dev->name,true) << "_MODULES_H */\n";
			out.close();
		}
